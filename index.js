...
app.put(`/api/users/:id`, userValidationMiddlewares, (req, res) => {
    ...
    return connection.query('UPDATE user SET ? WHERE id = ?', [updatedData, userId], (err, results) => {
      if (err) {
        return res.status(500).json({
          error: err.message,
          sql: err.sql,
      ...
      return connection.query('SELECT email, name FROM user WHERE id = ?', userId, (err2, records) => {
        if (err2) {
          return res.status(500).json({
            error: err2.message,
            sql: err2.sql,
